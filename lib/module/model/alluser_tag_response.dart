class AllUserForTagResponse {
    int? status;
    List<AllUsers>? data;
    String? message;

    AllUserForTagResponse({this.status, this.data, this.message});

    AllUserForTagResponse.fromJson(Map<String, dynamic> json) {
        status = json['status'];
        if (json['data'] != null) {
            data = <AllUsers>[];
            json['data'].forEach((v) {
                data!.add(new AllUsers.fromJson(v));
            });
        }
